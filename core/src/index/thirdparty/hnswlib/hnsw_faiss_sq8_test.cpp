...
void Sq8(const float *pdata, uint8_t *p_sq8, int dim, int nb) {
    float *vmax = (float*) malloc(sizeof(float) * dim);
    float *vmin = (float*) malloc(sizeof(float) * dim);
    float *vstep = (float*) malloc(sizeof(float) * dim);
    memset(vmax, 0, sizeof(float) * dim);
    memset(vmin, 0, sizeof(float) * dim);
    memset(vstep, 0, sizeof(float) * dim);

    for (int i = 0; i < dim; ++ i) {
        vmax[i] = vmin[i] = pdata[i];
    }

    for (int i = 1; i < nb; ++ i) {
        for (int j = 0; j < dim; ++ j) {
            vmax[j] = vmax[j] < pdata[i * dim + j] ? pdata[i * dim + j] : vmax[j];
            vmin[j] = vmin[j] > pdata[i * dim + j] ? pdata[i * dim + j] : vmin[j];
        }
    }

    for (int i = 0; i < dim; ++ i) {
        vstep[i] = (vmax[i] - vmin[i]) / 255;
    }

    memcpy(p_sq8, vmin, sizeof(float) * dim);
    memcpy(p_sq8 + sizeof(float) * dim, vstep, sizeof(float) * dim);
    int code_offset = sizeof(float) * dim * 2;

    for (int i = 0; i < nb; ++ i) {
        for (int j = 0; j < dim; ++ j) {
            uint8_t code = (uint8_t)((pdata[i * dim + j] - vmin[j] + vstep[j] / 2.0) / vstep[j]);
            memcpy(p_sq8 + code_offset + i * dim + j, &code, sizeof(uint8_t));
        }
    }
