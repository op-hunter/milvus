...
//    std::cout << std::endl;
    ...
    std::shared_ptr<HierarchicalNSW_NM<float>> hnsw_index = std::make_shared<HierarchicalNSW_NM<float>>(space, nb, dim, pqM, nbits, M, efConstruction);

    uint8_t *pq_codes = (uint8_t *) malloc(sizeof(float) * dim * (1 << nbits) + nb * pqM);
    ts = std::chrono::high_resolution_clock::now();
    hnsw_index->train(nb, pdata, pq_codes + nb * pqM);
    te = std::chrono::high_resolution_clock::now();
    std::cout << "sq_train costs: " << std::chrono::duration_cast<std::chrono::milliseconds>(te - ts).count() << "ms " << std::endl;

    hnsw_index->compute_codes(pdata, pq_codes, nb);
//    hnsw_index->set_sdc();
    printf("begin build\n");
    ts = std::chrono::high_resolution_clock::now();
//    hnsw_index->addPoint((void*)pdata, 0, 0, 0, pq_codes);
    hnsw_index->addPoint((void*)pdata, 0, 0, 0, 0);
    printf("begin parallel.\n");
#pragma omp parallel for
    for (int i = 1; i < nb; ++ i) {
//        auto f1 = std::chrono::high_resolution_clock::now();
//        std::cout << "=========================================================================================================================" << std::endl;
//        hnsw_index->addPoint((void*)pdata, i, 0, i, pq_codes);
//        auto f2 = std::chrono::high_resolution_clock::now();
//        std::cout << "the " << i << "th point has been added, total costs: " << std::chrono::duration_cast<std::chrono::milliseconds>(f2 - f1).count() << "ms " << std::endl;
//        std::cout << "=========================================================================================================================" << std::endl;
        hnsw_index->addPoint((void*)pdata, i, 0, i, 0);
//        hnsw_index->addPoint(pdata + i * dim, i);
    ...
    float *p_query = (float*)malloc(nq * dim * sizeof(float));
    memcpy(p_query, pdata, nq * dim * sizeof(float));
    free(pdata);
    std::cout << "p_query generated" << std::endl;
    sleep(3);
...
#pragma omp parallel for
    for (int i = 0; i < nq; ++ i) {
        std::vector<P> ret;
        ret = hnsw_index->searchKnn_NM((void*)(p_query + i * dim), topk, compare, nullptr, nullptr, pq_codes);
