...
namespace hnswlib_nm {
    ...
    using PQDC = float(*)(const uint8_t *, const float *);

    template<typename dist_t>
    class HierarchicalNSW_NM;

    float sdc(const HierarchicalNSW_NM<float> *, const uint8_t *, const float *);
    float adc(const HierarchicalNSW_NM<float> *, const uint8_t *, const float *);
    ...
    public:
        ...
        HierarchicalNSW_NM(SpaceInterface<dist_t> *s, size_t max_elements, size_t d, size_t pqM, size_t nbits, size_t M = 16, size_t ef_construction = 200,
                           size_t random_seed = 100) : link_list_locks_(max_elements), element_levels_(max_elements), pq_(d, pqM, nbits) {
            // linxj
            space = s;
...
//            label_offset_ = size_links_level0_;
        ...
        dist_t (*pqdc_)(const HierarchicalNSW_NM *inst, const uint8_t *, const float *) = adc;
        ...
        void train(size_t nb, const float *xb, uint8_t *p_trained) {
            pq_.train(nb, xb);
            memcpy(p_trained, pq_.centroids.data(), pq_.centroids.size() * sizeof(float));
        }

        void compute_codes(const float *xb, uint8_t *p_codes, size_t n) {
            pq_.compute_codes(xb, p_codes, n);
        }


        void set_sdc() {
            // todo: compute_sdc_table_ip()
            pq_.compute_sdc_table();
            printf("pq_.compute_sdc_table() finished.\n");
            pqdc_ = sdc;
        }

        dist_t cal_dist(const void *pa, const void *pb, const uint8_t *pq_codes, const tableint offset) const {
            // if pq_code is not null, calculate pq distance with pa, pq_code and offset, else calculate ordinary distance with pa, pb.
            return pq_codes ? pqdc_(this, pq_codes + offset * pq_.M, (const dist_t*)pa) : fstdistfunc_(pa, pb, dist_func_param_);
        }
        ...
        searchBaseLayer(tableint ep_id, const void *data_point, int layer, void *pdata, const uint8_t *pq_codes) {
...
//                dist_t dist = fstdistfunc_(data_point, getDataByInternalId(pdata, ep_id), dist_func_param_);
                dist_t dist = cal_dist(data_point, getDataByInternalId(pdata, ep_id), pq_codes, ep_id);
...
//                    dist_t dist1 = fstdistfunc_(data_point, currObj1, dist_func_param_);
                    dist_t dist1 = cal_dist(data_point, currObj1, pq_codes, candidate_id);
        ...
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
        searchBaseLayerST(tableint ep_id, const void *data_point, size_t ef, faiss::ConcurrentBitsetPtr bitset, void *pdata, const uint8_t *pq_codes) const {
            VisitedList *vl = visited_list_pool_->getFreeVisitedList();
            vl_type *visited_array = vl->mass;
            vl_type visited_array_tag = vl->curV;
...
//            _mm_prefetch(data_level0_memory_ + (*(data + 1)) * size_data_per_element_ + offsetData_, _MM_HINT_T0);
                ...
                for (size_t j = 1; j <= size; j++) {
                    ...
                    if (!(visited_array[candidate_id] == visited_array_tag)) {
                        ...
                        char *currObj1 = (getDataByInternalId(pdata, candidate_id));
                        dist_t dist = cal_dist(data_point, currObj1, pq_codes, candidate_id);
...
//                        if (!has_deletions || !isMarkedDeleted(candidate_id))
        ...
        void getNeighborsByHeuristic2(
                std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> &top_candidates,
                const size_t M, tableint *ret, size_t &ret_len, void *pdata, const uint8_t *pq_codes) {
            ...
            while (queue_closest.size()) {
                ...
                for (std::pair<dist_t, tableint> second_pair : return_list) {
                    // inner distance, either accuracy distance, adc and sdc are ok
                    // accuracy distance
                    dist_t curdist =
                            fstdistfunc_(getDataByInternalId(pdata, second_pair.second),
                                         ...
                                         dist_func_param_);;
//                    dist_t curdist =
//                            cal_dist(getDataByInternalId(pdata, second_pair.second),
//                                         getDataByInternalId(pdata, curent_pair.second),
//                                         pq_codes, second_pair.second);
//                    dist_t curdist =
//                            cal_dist(getDataByInternalId(pdata, second_pair.second),
//                                         getDataByInternalId(pdata, curent_pair.second),
//                                         pq_codes, curent_pair.second);
...
//        }
        ...
        void mutuallyConnectNewElement(const void *data_point, tableint cur_c,
                                       std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> &top_candidates,
                                       int level, void *pdata, const uint8_t *pq_codes) {

            size_t Mcurmax = level ? maxM_ : maxM0_;
            tableint *selectedNeighbors = (tableint*)malloc(sizeof(tableint) * M_);
            size_t selectedNeighbors_size = 0;
            getNeighborsByHeuristic2(top_candidates, M_, selectedNeighbors, selectedNeighbors_size, pdata, pq_codes);
...
//                                                dist_func_param_);
                    dist_t d_max = cal_dist(getDataByInternalId(pdata, cur_c), getDataByInternalId(pdata, selectedNeighbors[idx]),
                                                pq_codes, selectedNeighbors[idx]);
                    // Heuristic:
...
//                                             dist_func_param_), data[j]);
                        candidates.emplace(
                                cal_dist(getDataByInternalId(pdata, data[j]), getDataByInternalId(pdata, selectedNeighbors[idx]),
                                             pq_codes, selectedNeighbors[idx]), data[j]);
                    }

                    size_t indx = 0;
                    getNeighborsByHeuristic2(candidates, Mcurmax, data, indx, pdata, pq_codes);
...
//        markDeletedInternal(search->second);
        ...
        void addPoint(void *data_point, labeltype label, size_t base, size_t offset, const uint8_t *pq_codes) {
            addPoint(data_point, label,-1, base, offset, pq_codes);
        }

        tableint addPoint(void *data_point, labeltype label, int level, size_t base, size_t offset, const uint8_t *pq_codes) {
            tableint cur_c = 0;
            {
...
//                    dist_t curdist = fstdistfunc_(getDataByInternalId(data_point, (tableint)offset), getDataByInternalId(data_point, currObj), dist_func_param_);
                    dist_t curdist = cal_dist(getDataByInternalId(data_point, (tableint)offset), getDataByInternalId(data_point, currObj), pq_codes, currObj);
...
//                                dist_t d = fstdistfunc_(getDataByInternalId(data_point, tableint(offset)), getDataByInternalId(data_point, cand), dist_func_param_);
                                dist_t d = cal_dist(getDataByInternalId(data_point, tableint(offset)), getDataByInternalId(data_point, cand), pq_codes, cand);
                                if (d < curdist) {
                                    curdist = d;
                                    currObj = cand;
                                    changed = true;
                ...
                for (int level = std::min(curlevel, maxlevelcopy); level >= 0; level--) {
                    ...
                    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates = searchBaseLayer(
                            currObj, getDataByInternalId(data_point, (tableint)offset), level, data_point, pq_codes);
                    if (epDeleted) {
//                        top_candidates.emplace(fstdistfunc_(getDataByInternalId(data_point, (tableint)offset), getDataByInternalId(data_point, enterpoint_copy), dist_func_param_), enterpoint_copy);
                        top_candidates.emplace(cal_dist(getDataByInternalId(data_point, (tableint)offset), getDataByInternalId(data_point, enterpoint_copy), pq_codes, enterpoint_copy), enterpoint_copy);
                        if (top_candidates.size() > ef_construction_)
                            top_candidates.pop();
                    }
                    currObj = top_candidates.top().second;

                    mutuallyConnectNewElement(getDataByInternalId(data_point, (tableint)offset), cur_c, top_candidates, level, data_point, pq_codes);
                }
            } else {
        ...
        searchKnn_NM(const void *query_data, size_t k, faiss::ConcurrentBitsetPtr bitset, dist_t *pdata, const uint8_t *pq_codes) const {
            std::priority_queue<std::pair<dist_t, labeltype >> result;
            if (cur_element_count == 0) return result;

            tableint currObj = enterpoint_node_;
//            dist_t curdist = fstdistfunc_(query_data, getDataByInternalId(pdata, enterpoint_node_), dist_func_param_);
            dist_t curdist = cal_dist(query_data, getDataByInternalId(pdata, enterpoint_node_), pq_codes, enterpoint_node_);
...
//                        dist_t d = fstdistfunc_(query_data, getDataByInternalId(pdata, cand), dist_func_param_);
                        dist_t d = cal_dist(query_data, getDataByInternalId(pdata, cand), pq_codes, cand);
            ...
            if (bitset != nullptr) {
                std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
                        top_candidates1 = searchBaseLayerST<true>(currObj, query_data, std::max(ef_, k), bitset, pdata, pq_codes);
                top_candidates.swap(top_candidates1);
            }
            else{
                std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
                        top_candidates1 = searchBaseLayerST<false>(currObj, query_data, std::max(ef_, k), bitset, pdata, pq_codes);
                top_candidates.swap(top_candidates1);
            }
...
//            result.push(std::pair<dist_t, labeltype>(rez.first, getExternalLabel(rez.second)));
        ...
        searchKnn_NM(const void* query_data, size_t k, Comp comp, faiss::ConcurrentBitsetPtr bitset, dist_t *pdata, const uint8_t *pq_codes) {
            std::vector<std::pair<dist_t, labeltype>> result;
            if (cur_element_count == 0) return result;

            auto ret = searchKnn_NM(query_data, k, bitset, pdata, pq_codes);
    ...
    // only support l2 right now
    float sdc(const HierarchicalNSW_NM<float> *inst, const uint8_t *code, const float *x) {
        // todo: calculate symmetric distance
        auto t0 = faiss::getmillisecs();
        float dis = 0.0;
        const float *tab = inst->pq_.sdc_table.data();
        int64_t step = inst->pq_.ksub * inst->pq_.ksub;
        if (inst->pq_.sdc_table.size() != inst->pq_.M * step)
            throw std::runtime_error("pq_.sdc_table.size() != pq_.M * pq_.ksub * pq_.ksub!");
        uint8_t *xcode = (uint8_t*)malloc(inst->pq_.M);
        inst->pq_.compute_code(x, xcode);
//        printf("vector x:\n");
//        for (auto i = 0; i < inst->pq_.d; ++ i) {
//            printf("%.2f ", x[i]);
//        }
//        printf("\npq code of vector x:\n");
//        for (auto i = 0; i < inst->pq_.M; ++ i) {
//            printf("%d ", (int)xcode[i]);
//        }
//        printf("\n");
        for (auto i = 0; i < inst->pq_.M; ++ i) {
            dis += tab[code[i] + xcode[i] * inst->pq_.ksub];
            tab += step;
        }
        free(xcode);
//        printf("sdc dis = %.2f\n", dis);
//        auto t1 = faiss::getmillisecs();
//        printf("sdc calulate costs %.2lfms\n", t1 - t0);
        return dis;
    }

    float adc(const HierarchicalNSW_NM<float> *inst, const uint8_t *code, const float *x) {
        // todo: calculate asymmetric distance
        float dis = 0;
        float *approximate_vec = (float*) malloc(inst->pq_.d * sizeof(float));
        inst->pq_.decode(code, approximate_vec);
        dis = inst->fstdistfunc_(approximate_vec, x, inst->dist_func_param_);
        free(approximate_vec);
        return dis;
    }
