    ...
    */
    ...
    inline char *getDataByInternalId(void *pdata, tableint offset) const {
        if (is_sq8_) {
            ...
            float *ret = (float*) malloc(sizeof(float) * dim);
            float *pstart = getVstartByInternalId(pdata, offset);
            float *pstep = getVstepByInternalId(pdata, offset);
            uint8_t *pcode = getPcodeByInternalId(pdata, offset);
            for (auto i = 0; i < dim; ++ i) {
                ret[i] = pstart[i] + pstep[i] * (pcode + offset * dim)[i];
            }
            return (char*)ret;
        } else {
    ...
    inline float *getVstartByInternalId(void *pdata, tableint offset) const {
        return (float*) pdata;
    }

    inline float *getVstepByInternalId(void *pdata, tableint offset) const {
        return (float*)pdata + *(size_t *)dist_func_param_;
    }

    inline uint8_t *getPcodeByInternalId(void *pdata, tableint offset) const {
        return ((uint8_t *)((float*)pdata + *(size_t *)dist_func_param_ * 2));
    }
    ...
    searchBaseLayer(tableint ep_id, const void *data_point, int layer, void *pdata) {
        ...
        if (!isMarkedDeleted(ep_id)) {
            ...
            dist_t dist = fstdistfunc_(data_point, p_ep_id_data, dist_func_param_);
            if (is_sq8_)
                free(p_ep_id_data);
            ...
            for (std::pair<dist_t, tableint> second_pair : return_list) {
                ...
                dist_t curdist =
                        fstdistfunc_(p_second_pair_data,
                                     ...
                                     dist_func_param_);
                if (is_sq8_) {
                    free(p_second_pair_data);
                    free(p_current_pair_data);
                }
...
//        }
        ...
        for (size_t idx = 0; idx < selectedNeighbors_size; idx++) {
            ...
            if (sz_link_list_other < Mcurmax) {
                ...
                dist_t d_max = fstdistfunc_(p_curc_data, p_neighbor_data,
                                            dist_func_param_);
                // Heuristic:
                ...
                for (size_t j = 0; j < sz_link_list_other; j++) {
                    auto p_dataj = getDataByInternalId(pdata, data[j]);
                    candidates.emplace(
                            fstdistfunc_(p_dataj, p_neighbor_data,
...
//                }
    ...
    std::priority_queue<std::pair<dist_t, tableint>> searchKnnInternal(void *query_data, int k, dist_t *pdata) {
        ...
        auto p_ep_data = getDataByInternalId(pdata, enterpoint_node_);
        dist_t curdist = fstdistfunc_(query_data, p_ep_data, dist_func_param_);
        if (is_sq8_)
            free(p_ep_data);
        ...
        for (size_t level = maxlevel_; level > 0; level--) {
            ...
            while (changed) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    auto p_cand_data = getDataByInternalId(pdata, cand);
                    dist_t d = fstdistfunc_(query_data, p_cand_data, dist_func_param_);
                    if (is_sq8_)
                        free(p_cand_data);
