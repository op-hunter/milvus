...
//    std::cout << std::endl;
    ...
    std::shared_ptr<HierarchicalNSW_NM<float>> hnsw_index = std::make_shared<HierarchicalNSW_NM<float>>(space, nb, M, efConstruction);
...
#pragma omp parallel for
    for (int i = 0; i < nq; ++ i) {
        std::vector<P> ret;
        ret = hnsw_index->searchKnn_NM((void*)(pdata + i * dim), topk, compare, nullptr, pdata);
