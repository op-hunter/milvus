...
//            label_offset_ = size_links_level0_;
            ...
            mult_ = 1 / log(1.0 * M_);
            revSize_ = 1.0 / mult_;
            std::cout << "in constructor, current syc distance invoke times is: " << hnswlib::l2_dis_cnt << std::endl;
        }
        ...
        ~HierarchicalNSW() {
            ...
            // linxj: delete
            delete space;
            std::cout << "in deconstructor, distance invoke times is: " << hnswlib::l2_dis_cnt << std::endl;
        }
...
//        }
        ...
        linklistsizeint *get_linklist0(const char* addr, tableint internal_id) const {
            return (linklistsizeint *) (addr + internal_id * size_data_per_element_);
        };
...
//            result.push(std::pair<dist_t, labeltype>(rez.first, getExternalLabel(rez.second)));
        ...
        void indegree_stats() {
            ...
            for (auto i = 0; i <= max_in_degree; ++ i) {
                if (in_degree_stats_[i])
                    f << i << " " << in_degree_stats_[i] << std::endl, sum_ind += in_degree_stats_[i] * i;
            }
            f << "-----------------------------------------------------------" << std::endl;
            f << "out-degree stats: " << std::endl;
            for (auto i = 0; i <= M_ * 2 + 2; ++ i) {
                if (out_degree_stats_[i])
                    f << i << " " << out_degree_stats_[i] << std::endl, sum_outd += out_degree_stats_[i] * i;
            }
            f << "-----------------------------------------------------------" << std::endl;
            f << "sum of indegrees is: " << sum_ind << ", sum of outdegree is: " << sum_outd << std::endl;
            f.close();
        }
        ...
        /******************************************************************************
         * implementations inspired by ngt paper
         * cmli@2020.8.13
         ******************************************************************************/
        ...
        /**
         * algorithm 2 ConstructAdjustedGraph
         * input: G(V, E) = data_level0_memory_, eo = expected out-degree, ei = expected in-degree
        ...
        void ConstructAdjustedGraph(int eo, int ei, const float* pdata, char* output) {
            ...
            std::priority_queue<std::pair<dist_t, tableint>> neighbors;
            for (auto i = 0; i < max_elements_; ++ i) {
                auto data = get_linklist0(i);
                auto size = getListCount(data);
                for (auto j = 1; j <= size; ++ j) {
                    neighbors.emplace(
                            -fstdistfunc_(getDataByInternalId(pdata, data[j]), getDataByInternalId(pdata, i),
                                         dist_func_param_), data[j]);
                }
                int p = 1;
                while (!neighbors.empty() && (p <= eo || p <= ei)) {
                    auto closest = neighbors.top();
                    neighbors.pop();
                    if (p <= eo) {
                        ...
                        if (j > s_neis && s_neis < maxM0_) {
                            c_neis[++ s_neis] = closest.second;
                            setListCount(c_neis, s_neis);
                        }
                    }
                    if (p <= ei) {
        ...
        void ConstructAdjustedGraphWithConstraint(int eo, int ei, const float* pdata, char* output) {
            ...
            alloc_initial_space(output);
            char *Gi = nullptr;// Gi
            alloc_initial_space(Gi);
            char *Gt = nullptr;// Gt
            ConstructAdjustedGraph(0, ei, pdata, Gt); // Gt(Vt, Et) = ConstructAdjustedGraph(G, 0, ei)
            ...
            while (!vertex_set.empty()) {
                ...
                vertex_set.pop();
                auto data = get_linklist0(Gt, cur_v.second);
                auto size = getListCount(data);
            ...
            for (auto i = 0; i < max_elements_; ++ i) {
                auto data = get_linklist0(i);
                auto size = getListCount(data);
                auto ge_data = get_linklist0(output, i);
                auto ge_size = getListCount(ge_data);
                if (ge_size >= eo) continue;
                while (!neighbors_set.empty()) neighbors_set.pop();
                for (auto j = 1; j <= size; ++ j) {
                    neighbors_set.emplace(-fstdistfunc_(getDataByInternalId(pdata, data[j]),
                                                        getDataByInternalId(pdata, i),
                                                        dist_func_param_), data[j]);
                }
                while (!neighbors_set.empty() && ge_size < eo) {
                    ...
                    if (j > ge_size)
                        add_edge(output, i, cur_v.second);
                    ge_size = getListCount(ge_data);
                }
            }
