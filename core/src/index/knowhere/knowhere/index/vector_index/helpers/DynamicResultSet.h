...
struct RangeSearchSet {
    ...
    explicit RangeSearchSet(idx_t nq);
...
struct RangeQueryResult {
    idx_t qid; /// Query ID:id of the query
...
//    RangeSearchSet *res; /// final result container
    std::vector<RangeQueryResult> queries; /// partial query results
    ...
    RangeQueryResult& new_result(idx_t qid, size_t qnr);

    void copy_result (RangeSearchSet *res, std::vector<size_t> &lims, bool incremental = false);
...
struct DynamicResultSet {
    ...
    RangeSearchSet *res; /// final result container
    std::vector<size_t> query_boundaries; /// prefix of the size of each query's result set
